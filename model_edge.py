...
class bond_agg_layer(MessagePassing, nn.Module):
    ...
    def bond_agg(self, x, edge_index):
        ...
        edge_atten =  F.dropout(edge_atten, p=0.2, training=self.training)
        x = edge_atten*x
...
class one_hot(torch.nn.Module):
    ...
    def forward(self,data):
        tensor = torch.zeros(len(data),self.d_in).to(device)
        tensor[torch.arange(len(data)),data.squeeze()]=1
        return tensor

class Bond_Cycle_Network(MessagePassing, nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, name, dropout, task_type, num_classes):
        super(Bond_Cycle_Network, self).__init__()
        self.name = name
        if self.name == "ZINC":
            self.embedding = one_hot(28)
            self.edge_embedding = one_hot(4)
            self.input_dim = 60
        else:
            self.input_dim = 21
        ...
        self.mlp_feature =  nn.Sequential(
            nn.Linear(self.input_dim, in_channels), 
        ...
        self.edge_agg_layer1 = edge_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.edge_agg_layer2 = edge_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.edge_agg_layer3 = edge_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.edge_agg_layer4 = edge_agg_layer(hidden_channels, out_channels, self.dropout)
        self.bond_agg_layer1 = bond_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.cycle_agg_layer1 = cycle_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.bond_agg_layer2 = bond_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.cycle_agg_layer2 = cycle_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.bond_agg_layer3 = bond_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.cycle_agg_layer3 = cycle_agg_layer(hidden_channels, hidden_channels, self.dropout)
        self.bond_agg_layer4 = bond_agg_layer(hidden_channels, out_channels, self.dropout)
        self.cycle_agg_layer4 = cycle_agg_layer(hidden_channels, out_channels, self.dropout)
    ...
    def forward(self, sub_x, sub_edge_index, batch,cycle_info,y):
        if self.name == "ZINC":
            x = self.embedding(sub_x)
            y = self.edge_embedding(y)
        else:
            x = sub_x
            y = y

        x = torch.cat([x[sub_edge_index[0]],x[sub_edge_index[1]],y],dim=1)

        x = self.mlp_feature(x)

        bond = self.bond_agg_layer1(x, sub_edge_index)

        cycle = self.cycle_agg_layer1(x, cycle_info)

        x = self.edge_agg_layer1(x, bond, cycle, sub_edge_index, cycle_info)

        bond = self.bond_agg_layer2(x, sub_edge_index)
        
        cycle = self.cycle_agg_layer2(x, cycle_info)

        x = self.edge_agg_layer2(x, bond, cycle, sub_edge_index, cycle_info)

        bond = self.bond_agg_layer3(x, sub_edge_index)
        
        cycle = self.cycle_agg_layer3(x, cycle_info)

        x = self.edge_agg_layer3(x, bond, cycle, sub_edge_index, cycle_info)

        bond = self.bond_agg_layer4(x, sub_edge_index)
        
        cycle = self.cycle_agg_layer4(x, cycle_info)

        x = self.edge_agg_layer4(x, bond, cycle, sub_edge_index, cycle_info)
